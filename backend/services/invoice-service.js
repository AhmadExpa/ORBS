...
const invoiceDisclaimer =
  "This invoice is generated by ElevenOrbits for managed services and related account charges. Service delivery, provisioning, renewals, cancellations, and support remain subject to ElevenOrbits service terms and any active Managed Service Agreement. A paid new-order invoice confirms receipt of an advance payment, not acceptance or provisioning of the service request. Requests rejected during pre-provisioning review are refunded through their original card or wallet source. Paid invoices do not transfer ownership of licensed software, third-party infrastructure, IP addresses, domains, or provider-managed resources. Taxes, duties, banking fees, currency conversion charges, and third-party processing fees remain the customer's responsibility unless explicitly stated otherwise.";
...
function drawStatusBadge(doc, status, x, y) {
  const tone = {
    ...
    refunded: { background: "#dbeafe", text: "#1d4ed8" },
...
export async function generateInvoicePdf({ invoice, customer, planName, supportEmail }) {
  ...
  const pdfBuffer = await new Promise((resolve, reject) => {
    ...
    const summaryRows = [
      ...
      [invoice.status === "refunded" ? "Originally Paid On" : "Paid On", ["paid", "refunded"].includes(invoice.status) ? formatDate(invoice.paidAt) : "Pending"],
      ["Payment Method", paymentMethod],
      ["Payment Reference", invoice.paymentReferenceCode || "Pending"],
      ...(invoice.status === "refunded" ? [["Refund Reference", invoice.metadata?.refundReference || "Pending"]] : []),
    ...
    const paymentNote = invoice.status === "paid"
      ? `Payment has been verified through ${paymentMethod}.`
      : invoice.status === "refunded"
        ? `The advance payment was refunded through ${paymentMethod}. Refund reference ${invoice.metadata?.refundReference || "not available"}.`
    ...
    drawKeyValue(doc, {
      ...
      y: cursorY + 50,
      width: 174,
      label: invoice.status === "paid" ? "Amount Paid" : invoice.status === "refunded" ? "Amount Refunded" : invoice.status === "void" ? "Amount Voided" : "Amount Due",
