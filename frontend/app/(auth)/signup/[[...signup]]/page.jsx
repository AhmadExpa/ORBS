...
import { ArrowRight, CreditCard, LoaderCircle, ServerCog, SlidersHorizontal } from "lucide-react";
...
export default async function SignupPage({ searchParams }) {
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(135deg,#f8fafc_0%,#ffffff_46%,#eef2f6_100%)] px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto grid min-h-[calc(100vh-3rem)] w-full max-w-6xl overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_36px_110px_-72px_rgba(15,23,42,0.55)] lg:grid-cols-[1.05fr_0.95fr]">
        <section className="flex items-center justify-center p-5 sm:p-8 lg:p-12">
          <div className="w-full max-w-md">
            ...
            <SignedOut>
              <div className="mb-6">
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-400">Start Securely</p>
                <h2 className="mt-3 text-3xl font-semibold tracking-[-0.035em] text-slate-950">Create your account first.</h2>
              </div>
              <SignUp
                routing="path"
                path="/signup"
              ...
              <p className="mt-5 text-center text-sm text-slate-600">
                ...
                <Link href={loginUrl} className="inline-flex items-center gap-1 font-semibold text-slate-950 transition hover:text-slate-700">
            ...
            <SignedIn>
              <div className="flex flex-col items-center justify-center rounded-lg border border-slate-200 bg-white px-6 py-14 text-center shadow-sm">
                ...
                <p className="mt-5 text-lg font-semibold tracking-tight text-slate-900">Setting up your account…</p>
        ...
        <section className="relative hidden overflow-hidden bg-slate-950 p-10 text-white lg:block">
          ...
          <div className="relative flex h-full flex-col justify-between">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.28em] text-white/45">Protected Purchase Flow</p>
              <h1 className="mt-5 max-w-xl text-5xl font-semibold leading-[0.98] tracking-[-0.04em]">
                Account first. Settings next. Card payment last.
              </h1>
              <p className="mt-6 max-w-md text-sm leading-7 text-white/64">
                Your selected plan continues into configuration after signup, then checkout stays inside the portal.
              </p>
            </div>
            <div className="grid gap-3">
              {[
                { icon: SlidersHorizontal, label: "Configure settings", value: "Choose region, storage, image, features, and notes after account creation." },
                { icon: CreditCard, label: "Pay by card", value: "Complete the first purchase with card checkout in the protected portal." },
                { icon: ServerCog, label: "Open the portal", value: "After success, continue to your dashboard for service status and support." },
              ].map((item) => {
