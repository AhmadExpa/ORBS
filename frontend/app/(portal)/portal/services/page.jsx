...
function getServiceSection(subscription) {
  ...
  if (EDGE_STORAGE_CATEGORY_SLUGS.has(slug)) {
    return "edge-storage";
  }
  ...
  return (
    <Card className="overflow-hidden border-cyan-200">
      <CardHeader className="bg-gradient-to-r from-cyan-50 via-white to-emerald-50">
        <div className="flex items-start gap-3">
          <Cloud className="mt-1 h-6 w-6 text-cyan-700" />
          <div>
      ...
      <CardContent className="grid gap-4 p-6 lg:grid-cols-2">
        {subscriptions.map((subscription) => {
          ...
          return (
            <div key={subscription._id} className="rounded-[1.75rem] border border-cyan-100 bg-gradient-to-br from-white via-cyan-50/60 to-emerald-50/60 p-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-xs uppercase tracking-[0.18em] text-cyan-700">Delivery & Storage</p>
                  <p className="mt-2 text-xl font-semibold text-slate-950">{subscription.productPlanId?.name || "Edge Service"}</p>
                  <p className="mt-1 text-sm text-slate-500">{getCategoryName(subscription)}</p>
                </div>
                <StatusBadge status={subscription.status} />
              </div>

              <div className="mt-5 rounded-3xl border border-cyan-100 bg-white/85 p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Managed Stack</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {stack.length ? (
                    stack.map((item) => (
                    ...
                    <span className="text-sm text-slate-500">Service details will appear after provisioning.</span>
                  )}
                </div>
              </div>
              ...
              {details.length ? (
                <div className="mt-4 space-y-2">
                  {details.map((detail) => (
                    <div key={`${detail.label}-${detail.value}`} className="flex items-center justify-between gap-4 rounded-2xl border border-cyan-100 bg-white/85 px-4 py-3 text-sm">
                      <span className="text-slate-500">{detail.label}</span>
